...
class VideoLoader:
    ...
    def download(self) -> None:
        if match(r"http(s)?://(www\.)?speedrun.com/fantasy_life/runs/[a-z0-9]{8}", self.link):
        ...
        if match(r"http(s)?://(www\.)?(youtube\.com/watch\?v=|youtu\.be/)[0-9A-Za-z_-]{11}", self.link):
            ...
            if not path.exists(self.link) or input("Seems like this video is already on your computer. Would you still like to download it again? (Y/N)\n\t").upper() == "Y":
                yt.streams.filter(resolution="360p", fps=30).first().download(f"{path.dirname(path.realpath(__file__))}\\inputs", clean_title, skip_existing=False)
